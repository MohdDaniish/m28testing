...
router.get("/communitytree", async (req, res) => {
  try {
    ...
    const matrixstruct = await m28userplace.find({ referrer: user , packageId : poolid }).limit(2);
    const mergedRecords = [];
    ...
    for (const record of matrixstruct) {
      ...
      if (userRecord) {
        const rankValue = await contract.methods
          .currentRank(record.user, record.packageId)
          .call();
        const rankLabel = rankMap[parseInt(rankValue)] || "unranked";
        ...
        for (const subChild of subChildren) {
          ...
          if (subChildUserRecord) {
            const subRankValue = await contract.methods
              .currentRank(subChild.user, subChild.packageId)
              .call();
            const subRankLabel = rankMap[parseInt(subRankValue)] || "unranked";

            const mergedSubChild = {
              ...subChild.toObject(),
              userId: subChildUserRecord.userId,
              rank: subRankLabel,
            };
    ...
    res.status(200).json({ matrixstruct: mergedRecords });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Server error" });
  }
...
router.get('/autopoolincome', async (req, res) => {
  ...
  try {
      // autopool income
      const autores = await newuserplace.aggregate([
  {
    $match: { ref_id: id }
  },
  {
    $lookup: {
      from: "registration", // collection name (check your actual MongoDB collection name, it may be plural)
      localField: "user",
      foreignField: "user",
      as: "userDetails"
    }
  ...
  },
  {
    $project: {
      ...
      user: 1,
      ref_id: 1,
      userId: "$userDetails.userId"  // include the userId from registration
    }
  }
]);
...
router.get('/view-user-records', async (req, res) => {
  const {id} = req.query
  try {
     const autores = await newuserplace.aggregate([
  {
    $match: { ref_id: Number(id) }
  },
  {
    $lookup: {
      from: "Registration", // collection name (check your actual MongoDB collection name, it may be plural)
...
router.get("/getDetailsByLevel", async (req, res) => {
  try {
    ...
    const result = await m28userplace.aggregate([
      {
        $match: { user: address }
      },
      {
